...
сервер. В случае падения бекенда балансировщик должен распределить трафик на остальные сервера. Также nginx сам может
определять недоступность бекенда по timeout.
...
Терминация SSL происходит за счет nginx.
