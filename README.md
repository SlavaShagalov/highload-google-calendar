...
![Логическая схема БД](img/db_scheme-LOGIC.png)
...
![Физическая схема БД](img/db_scheme-PHYSIC.png)

Основным хранилищем будет служить популярная среди разработчиков СУБД Postgres. Имеет высокую надежность и
расширяемость.

Для хранения уведомлений можно использовать Redis. Redis хранит все данные в оперативной памяти и способен отдавать
данные быстрее реляционных СУБД. Он будет служить в качестве очереди уведомлений, а также временным хранилищем
напоминаний. Также в Redis можно хранить сессии пользователей.
...
| Технология |       Область применения       |                         Обоснавание                          |
|------------|:------------------------------:|:------------------------------------------------------------:|
| Typescript |          ЯП Frontend           |         Типизация, отлов ошибок на этапе компиляции          |
...
| Redis      | Хранилище сессий и уведомлений |                    скорость, популярность                    |

## 9. Схема проекта

![Схема проекта](img/db_scheme-project_scheme.drawio.png)
...
Cобытия публикуются в брокер сообщений. Сonsumer-ы на другой стороне асинхронно обрабатывают события.
